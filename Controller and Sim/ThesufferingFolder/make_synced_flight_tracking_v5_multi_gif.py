#!/usr/bin/env python3
"""
make_synced_flight_tracking_v5_multi_gif.py

V5 version of the synchronized flight/tracking animation script.

It uses one simulation CSV as the single source of truth, then creates:

1. Combined flight + tracking animation:
      mpc_v5_csv_synced_flight_tracking.mp4
      mpc_v5_csv_synced_flight_tracking.gif

2. Plot-only tracking GIF:
      mpc_v5_tracking_plots_only.gif

3. 3D flight-only GIF:
      mpc_v5_3d_flight_only.gif

4. Downsampled / smaller combined GIF:
      mpc_v5_csv_synced_flight_tracking_small.gif

Default input:
      mpc_output_plots_v5/sim_out_MLP_MPC_v5.csv
...
CSV_PATH = "mpc_output_plots_v5/sim_out_MLP_MPC_v5.csv"
CSV_FALLBACK_PATH = "sim_out_MLP_MPC_v5.csv"

OUTPUT_DIR = "mpc_output_plots_v5/synced_csv_animation_v5"
OUTPUT_BASENAME = "mpc_v5_csv_synced_flight_tracking"
...
def resolve_csv_path():
    ...
    raise FileNotFoundError(
        ...
        f"  {primary}\n"
        f"  {fallback}\n"
        f"Edit CSV_PATH at the top of this script if your v5 CSV has a different name."
    )
...
def load_csv_data():
    ...
    # Command columns with v5 fallback:
    # 0-10 s: heading 50 deg
    # 10-20 s: heading 140 deg
    # pitch 3 deg, roll 0 deg
    if "psi_cmd_deg" not in df.columns:
        df["psi_cmd_deg"] = np.where(df["t"].to_numpy(dtype=float) < 10.0, 50.0, 140.0)
    if "theta_cmd_deg" not in df.columns:
        df["theta_cmd_deg"] = 3.0
...
def setup_tracking_axes(ax_track, ax_err, arr):
    ...
    ax_track.legend(loc="upper right", fontsize=8, framealpha=0.9)
...
def make_combined_animation(df, figsize=FIGSIZE):
    ...
    fig.suptitle("Synchronized MPC v5 Flight Animation and Attitude Tracking", fontsize=13, y=0.975)

    flight_objs = setup_flight_axis(ax_3d, df, mesh_body, arr, "MPC v5 Flight Animation")
...
def make_plot_only_animation(df):
    ...
    fig.suptitle("MPC v5 Attitude Tracking Animation", fontsize=13)
...
def make_flight_only_animation(df):
    ...
    fig.suptitle("MPC v5 3D Flight Animation", fontsize=13)
...
def export_all(df_anim):
    ...
    plot_only_gif = out_dir / "mpc_v5_tracking_plots_only.gif"
    flight_only_gif = out_dir / "mpc_v5_3d_flight_only.gif"

    # Combined MP4 + full GIF.
