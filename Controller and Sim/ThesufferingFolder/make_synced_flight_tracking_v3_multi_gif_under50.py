...
def export_all(df_anim):
    ...
    # Additional optimized combined GIF targeted below 50 MB.
    save_gif_under_target_size(combined_anim, combined_under50_gif, target_mb=TARGET_GIF_SIZE_MB)
    ...
    save_gif_under_target_size(plot_anim, plot_only_under50_gif, target_mb=TARGET_GIF_SIZE_MB)
    plt.close(plot_fig)

    # 3D-only GIF.
    flight_anim, flight_fig = make_flight_only_animation(df_anim)
    save_gif(flight_anim, flight_only_gif, fps=GIF_FPS, dpi=DPI_GIF)
    save_gif_under_target_size(flight_anim, flight_only_under50_gif, target_mb=TARGET_GIF_SIZE_MB)
