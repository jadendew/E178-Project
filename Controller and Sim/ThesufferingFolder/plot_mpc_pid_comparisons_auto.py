...
Outputs:
    mpc_pid_comparison_outputs/
        ...
        animations/<case>/
            <case>_3d_comparison.mp4 or .gif fallback
...
ROOT_DIR = Path(".")
OUTPUT_ROOT = Path("mpc_pid_comparison_outputs")
...
DEFAULT_HEADING_CMD_1_DEG = 50.0
DEFAULT_HEADING_CMD_2_DEG = 140.0
DEFAULT_PITCH_CMD_DEG = 3.0
...
def build_data(df, label, t_common):
    ...
    # Commands: prefer actual logged commands. Otherwise use v5 fallback.
    ...
    if heading_cmd is None:
        ...
        heading_src = "fallback v5 50->140 deg step"
    else:
    ...
    # Convert rates from rad/s to deg/s when present.
    for rate in ["p", "q", "r"]:
        if rate in data:
...
def make_3d_comparison_animation(case_name, mpc_df, pid_df, out_dir):
    ...
    try:
        ...
        try:
            print(f"Saving GIF fallback: {gif_path}")
            anim.save(gif_path, writer=PillowWriter(fps=ANIM_FPS), dpi=110)
            print(f"Saved animation fallback: {gif_path}")
