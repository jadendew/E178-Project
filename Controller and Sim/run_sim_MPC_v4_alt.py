...
OUTPUT_CSV = r"sim_out_MLP_MPC_v4.csv"
...
HEADING_CMD_DEG = 0.0  # v4: steady heading command, matched to initial yaw to avoid yaw transients
PITCH_CMD_DEG = 0.0
ROLL_CMD_DEG = 0.0
BETA_CMD_DEG = 0.0
...
# Time-varying command profile
#   v4 isolates the pitch-sine test:
#   0-15 s: hold heading steady and command only the pitch-direction sine.
#
# The MPC tracks pitch angle, not vertical position directly, so the requested
# 1 m amplitude vertical/pitch-direction sine is converted into an equivalent
# pitch command using the sine-path slope and the nominal forward speed.
USE_COMMAND_PROFILE = True
CMD_SINE_DURATION_S = 15.0
CMD_SINE_FREQ_HZ = 0.5
CMD_SINE_AMPLITUDE_M = 1.0

# Forward speed used to convert the vertical sine path slope into a pitch angle.
...
# MPC settings
# Final-damped version: prioritizes heading tracking while adding extra yaw/overall damping.
# Increase HORIZON / NUM_CANDIDATES later if you want more optimization quality.
MPC_HORIZON = 4
MPC_NUM_CANDIDATES = 30
...
MPC_Q_PITCH = 3.0
MPC_Q_ROLL = 0.95
MPC_Q_RATES = [0.105, 0.085, 0.230]
MPC_Q_YAW_RATE_PRED = 0.028  # direct predicted-heading-rate damping in MPC rollout
MPC_Q_TERMINAL_HEADING = 26.0  # harden final heading in MPC horizon
MPC_Q_TERMINAL_PITCH = 1.0
MPC_Q_TERMINAL_ROLL = 1.0
MPC_Q_ALPHA = 0.25
MPC_Q_BETA = 0.50
MPC_R_CONTROL = [0.028, 0.028]
MPC_R_DELTA_CONTROL = [0.62, 0.62]
...
MPC_LOCAL_SIGMA_DEG = [2.2, 2.2]
MPC_WARM_SIGMA_DEG = [0.55, 0.55]
...
MPC_HEADING_DEADBAND_DEG = 0.22
MPC_PITCH_DEADBAND_DEG = 0.12
MPC_ROLL_DEADBAND_DEG = 0.45
MPC_ALPHA_DEADBAND_DEG = 0.25
MPC_BETA_DEADBAND_DEG = 0.25
MPC_DEADBAND_INSIDE_SCALE = 0.65  # soft deadband: inside-band errors still count at 10% strength

MPC_USE_STEADY_HOLD_RULE = True
MPC_HOLD_CMD_CHANGE_DEADBAND_DEG = 0.08
MPC_HOLD_MIN_IMPROVEMENT_FRAC = 0.004
MPC_HOLD_MAX_HEADING_ERR_DEG = 0.25  # never hold a stale command if heading error is larger than this
...
HEADING_TO_ROLL_GAIN = 0.36      # deg roll command per deg heading error during large heading changes
HEADING_TO_ROLL_DEADBAND_DEG = 0.30  # below this, heading error creates zero roll command
...
SAVE_PLOTS = True
PLOT_OUTPUT_DIR = "mpc_output_plots_v4"
PLOT_DPI = 200

SAVE_ANIMATION = True
ANIMATION_OUTPUT_DIR = "mpc_output_animation_v4"
ANIMATION_FILENAME = "mpc_v4_3d_animation.mp4"  # falls back to .gif if mp4 fails
PITCH_ANIMATION_FILENAME = "mpc_v4_pitch_path_stl_animation.mp4"  # falls back to .gif if mp4 fails
ANIMATION_FPS = 30
ANIMATION_DPI = 140
...
def _extract_time_from_controller_call(args, kwargs) -> float | None:
    """
    Make the command-profile wrapper robust to different controller signatures.
    It tries common locations for simulation time, then falls back to a call counter.
    """
    if "t" in kwargs:
        try:
...
def make_command_profile_controller(controller_fn, cfg):
    """
    Wrap the existing MPC controller so the rest of the project does not need to
    be changed. Before every controller call, this updates cfg.heading_cmd_deg
    and cfg.pitch_cmd_deg.
    """
    cfg._command_profile_call_count = 0

    def controller_with_command_profile(*args, **kwargs):
        if bool(USE_COMMAND_PROFILE):
            t_now = _extract_time_from_controller_call(args, kwargs)
            if t_now is None:
                t_now = float(cfg._command_profile_call_count) * float(cfg.dt)
            cfg.heading_cmd_deg = float(command_profile_heading_deg(t_now, cfg))
            cfg.pitch_cmd_deg = float(command_profile_pitch_deg(t_now, cfg))
            cfg._command_profile_call_count += 1
        return controller_fn(*args, **kwargs)
...
def add_tracking_columns(df: pd.DataFrame, cfg) -> pd.DataFrame:
    ...
    df["x_rel_m"] = df["x"].to_numpy() - float(df["x"].iloc[0])
    df["z_up_rel_m"] = df["altitude_m"].to_numpy() - float(df["altitude_m"].iloc[0])
...
def animate_pitch_path_stl(df: pd.DataFrame, stride=10, interval_ms=30, trail_len=200,
                           ...
                           stl_rotation_offset_deg=(0.0, 0.0, 0.0)):
    """
    Create a second animation in the same visual style as the 3D animation.

    This version uses a fixed world-frame side view, but with a scrolling window
    centered on the aircraft:
      - the camera is orthogonal to the WORLD y-axis
      - x-limits are always plane_x +/- 25 m
      - z-limits are always plane_z +/- 5 m
      - the actual vertical motion is detrended to remove the net glider sink,
        so the oscillatory pitch-response can be compared against the commanded
        trajectory more directly
      - the visible box aspect is set to be very wide in x, with an 18:1 x:y
        aspect while preserving the 50 m by 10 m x:z window relationship
    """
    import matplotlib.pyplot as plt
    from matplotlib.animation import FuncAnimation
    from mpl_toolkits.mplot3d.art3d import Poly3DCollection
    ...
    # Relative-to-start actual motion in world coordinates.
    x0, y0, z0 = float(x[0]), float(y[0]), float(z[0])
    xp = x - x0
    yp = y - y0
    zp_raw = -(z - z0)

    # Remove only the net sink trend so the glider's oscillatory response can be
    # compared to the commanded trajectory without the overall gravity-driven drop.
    ...
    # Commanded path in WORLD frame using the commanded heading, not body frame.
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.set_title("Pitch Sine Command Path vs Aircraft Response")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_zlabel("Z up (m)")
    ax.set_proj_type("ortho")

    # Full-path reference plus progressive drawing.
    ax.plot(
    ...
    # Small body axes so they do not clutter the view.
    ...
    # Fixed y-limits from the full run, but a scrolling x/z window centered on the plane.
    ...
    # Requested scrolling windows.
    x_half_window = 25.0
    z_half_window = 5.0

    # Wide visual aspect in x; x:y = 18:1, and x:z = 50:10 = 5:1.
    ax.set_box_aspect((18.0, 1.0, 3.6))

    # Fixed world-frame view: look along the WORLD y-axis.
    ...
    def update(i):
        ...
        # Scroll the x and z windows with the aircraft centered in view.
...
def main():
    ...
    # Create both animations if either you want to display them or save them.
    ...
    # Save both animations as MP4, or GIF fallback.
    if SAVE_ANIMATION:
        save_animation_file(
            ani_3d,
