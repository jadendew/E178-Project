...
OUTPUT_CSV = r"sim_out_GainSchedule_v5.csv"
...
# Time-varying command profile copied from the MPC v5 script:
#   0-10 s: hold the original commanded heading/pitch
...
# Saving outputs
SAVE_PLOTS = True
PLOT_OUTPUT_DIR = "gain_schedule_output_plots_v5"
PLOT_DPI = 200

SAVE_ANIMATION = True
ANIMATION_OUTPUT_DIR = "gain_schedule_output_animation_v5"
ANIMATION_FILENAME = "gain_schedule_v5_animation.mp4"  # falls back to .gif if ffmpeg/mp4 saving fails
ANIMATION_FPS = 30
ANIMATION_DPI = 140
...
def command_profile_heading_deg(t: float, cfg=None) -> float:
    """
    Time-varying yaw/heading command copied from MPC v5.

    0-10 s: hold HEADING_CMD_DEG.
    10-20 s: step heading by CMD_FINAL_HEADING_STEP_DEG and hold it.
    """
    ...
    return _wrap_deg_series(np.array([psi0 + float(CMD_FINAL_HEADING_STEP_DEG)], dtype=float))[0]


def command_profile_pitch_deg(t: float, cfg=None) -> float:
    """
    MPC v5 pitch command: hold the original steady pitch command for the full run.
    No sine-wave pitch command is applied.
    """
...
def make_command_profile_controller(controller_fn, cfg):
    """
    Wrap the gain-scheduled controller so the controller receives the same
    time-varying heading/pitch commands as the MPC v5 script.
    """
...
def add_heading_tracking_columns(df: pd.DataFrame, cfg) -> pd.DataFrame:
    ...
    """
    # ---------------------------
    # Commands. If the MPC-v3 command profile is enabled, reconstruct the
    # time-varying commands for plotting/logging. Otherwise, use cfg constants.
    # ---------------------------
    if bool(USE_COMMAND_PROFILE):
...
def main():
    ...
    # Save plots and animation before plt.show().
    if SAVE_PLOTS:
