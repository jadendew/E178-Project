...
def simulate(
    ...
    for i in range(n_steps + 1):
        ...
        rows.append({
            ...
            # ============================================================
            # Controller gain-scheduling diagnostics
            # These are written by controllers/heading_pitch_hold.py
            # and copied here into the output dataframe / CSV.
            # ============================================================

            # Scheduled heading / roll gains
            "ctrl_K_PSI": float(getattr(cfg, "_ctrl_K_PSI", np.nan)),
            "ctrl_K_PHI": float(getattr(cfg, "_ctrl_K_PHI", np.nan)),
            "ctrl_K_P": float(getattr(cfg, "_ctrl_K_P", np.nan)),
            "ctrl_K_R": float(getattr(cfg, "_ctrl_K_R", np.nan)),

            # Scheduled pitch gains
            "ctrl_K_THETA": float(getattr(cfg, "_ctrl_K_THETA", np.nan)),
            "ctrl_K_Q": float(getattr(cfg, "_ctrl_K_Q", np.nan)),
            "ctrl_K_QD": float(getattr(cfg, "_ctrl_K_QD", np.nan)),

            # Scheduling variables
            "ctrl_V": float(getattr(cfg, "_ctrl_V", np.nan)),
            "ctrl_alpha_deg": float(getattr(cfg, "_ctrl_alpha_deg", np.nan)),
            "ctrl_beta_deg": float(getattr(cfg, "_ctrl_beta_deg", np.nan)),

            # Scale factors used by the schedule
            "ctrl_vel_scale": float(getattr(cfg, "_ctrl_vel_scale", np.nan)),
            "ctrl_aero_scale": float(getattr(cfg, "_ctrl_aero_scale", np.nan)),
            "ctrl_pitch_err_scale": float(getattr(cfg, "_ctrl_pitch_err_scale", np.nan)),
            "ctrl_roll_err_scale": float(getattr(cfg, "_ctrl_roll_err_scale", np.nan)),
            "ctrl_heading_err_scale": float(getattr(cfg, "_ctrl_heading_err_scale", np.nan)),
            "ctrl_roll_damp_scale": float(getattr(cfg, "_ctrl_roll_damp_scale", np.nan)),
            "ctrl_pitch_damp_scale": float(getattr(cfg, "_ctrl_pitch_damp_scale", np.nan)),
            # Derivative-based damping diagnostics
            "ctrl_p_dot_deg_s2": float(getattr(cfg, "_ctrl_p_dot_deg_s2", np.nan)),
            "ctrl_q_dot_deg_s2": float(getattr(cfg, "_ctrl_q_dot_deg_s2", np.nan)),
            "ctrl_r_dot_deg_s2": float(getattr(cfg, "_ctrl_r_dot_deg_s2", np.nan)),

            "ctrl_roll_accel_damp_scale": float(getattr(cfg, "_ctrl_roll_accel_damp_scale", np.nan)),
            "ctrl_pitch_accel_damp_scale": float(getattr(cfg, "_ctrl_pitch_accel_damp_scale", np.nan)),
            "ctrl_yaw_accel_damp_scale": float(getattr(cfg, "_ctrl_yaw_accel_damp_scale", np.nan)),

            # Controller errors
            "ctrl_e_psi_deg": float(getattr(cfg, "_ctrl_e_psi_deg", np.nan)),
            "ctrl_e_phi_deg_pre": float(getattr(cfg, "_ctrl_e_phi_deg_pre", np.nan)),
            "ctrl_e_phi_deg": float(getattr(cfg, "_ctrl_e_phi_deg", np.nan)),
            "ctrl_e_theta_deg": float(getattr(cfg, "_ctrl_e_theta_deg", np.nan)),

            # Internal controller commands
            "ctrl_phi_cmd_deg": float(getattr(cfg, "_ctrl_phi_cmd_deg", np.nan)),
            "ctrl_q_cmd_deg_s": float(getattr(cfg, "_ctrl_q_cmd_deg_s", np.nan)),

            # Commands before and after rate limiting
            "ctrl_deltaS_unsmoothed_deg": float(getattr(cfg, "_ctrl_deltaS_unsmoothed_deg", np.nan)),
            "ctrl_deltaD_unsmoothed_deg": float(getattr(cfg, "_ctrl_deltaD_unsmoothed_deg", np.nan)),
            "ctrl_deltaS_final_deg": float(getattr(cfg, "_ctrl_deltaS_final_deg", np.nan)),
            "ctrl_deltaD_final_deg": float(getattr(cfg, "_ctrl_deltaD_final_deg", np.nan)),
            "ctrl_q_cmd_raw_deg_s": float(getattr(cfg, "_ctrl_q_cmd_raw_deg_s", np.nan)),
            "ctrl_phi_cmd_raw_deg": float(getattr(cfg, "_ctrl_phi_cmd_raw_deg", np.nan)),

            "ctrl_deltaS_raw_no_clamp_deg": float(getattr(cfg, "_ctrl_deltaS_raw_no_clamp_deg", np.nan)),
            "ctrl_deltaD_raw_no_clamp_deg": float(getattr(cfg, "_ctrl_deltaD_raw_no_clamp_deg", np.nan)),

            "ctrl_q_cmd_saturated": float(getattr(cfg, "_ctrl_q_cmd_saturated", np.nan)),
            "ctrl_phi_cmd_saturated": float(getattr(cfg, "_ctrl_phi_cmd_saturated", np.nan)),
            "ctrl_deltaS_saturated": float(getattr(cfg, "_ctrl_deltaS_saturated", np.nan)),
            "ctrl_deltaD_saturated": float(getattr(cfg, "_ctrl_deltaD_saturated", np.nan)),
            "ctrl_use_derivative_damping": float(getattr(cfg, "_ctrl_use_derivative_damping", np.nan)),
        })
        # Advance
        if i < n_steps:
            state = next_state
