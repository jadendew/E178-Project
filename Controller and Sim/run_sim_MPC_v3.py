...
OUTPUT_CSV = r"sim_out_MLP_MPC_v3.csv"
...
HEADING_CMD_DEG = 50.0  # default/original heading command; command profile starts by holding this
PITCH_CMD_DEG = 3.0
ROLL_CMD_DEG = 0.0
BETA_CMD_DEG = 0.0
...
#   0-5 s: hold the script's original commanded heading/pitch
#   5-20 s: command a vertical/pitch-direction sine-path tangent
#            z_up_cmd = A sin(2*pi*f*tau), with 1.0 m amplitude
#            yaw/heading remains fixed at HEADING_CMD_DEG during this segment
#   20-30 s: command a new steady yaw/heading 90 deg from HEADING_CMD_DEG
#            and hold the original steady pitch command
USE_COMMAND_PROFILE = True
CMD_HOLD_INITIAL_S = 5.0
CMD_SINE_DURATION_S = 15.0
CMD_SINE_FREQ_HZ = 0.25
CMD_SINE_AMPLITUDE_M = 1.0
CMD_FINAL_HEADING_STEP_DEG = 90.0
# Forward speed used to convert the vertical sine path slope into a pitch angle.
# This is only used to generate the pitch command because this MPC tracks pitch,
# not vertical position directly.
...
SAVE_PLOTS = True
PLOT_OUTPUT_DIR = "mpc_output_plots_v3"
PLOT_DPI = 200

SAVE_ANIMATION = True
ANIMATION_OUTPUT_DIR = "mpc_output_animation_v3"
ANIMATION_FILENAME = "mpc_v3_animation.mp4"  # falls back to .gif if mp4 fails
ANIMATION_FPS = 30
ANIMATION_DPI = 140
...
def command_profile_heading_deg(t: float, cfg=None) -> float:
    """
    Time-varying yaw/heading command for this 30 s case.

    0-5 s: hold the script's original heading command, HEADING_CMD_DEG.
    5-20 s: keep that same yaw command while the pitch command gets the sine input.
    20-30 s: step yaw/heading by CMD_FINAL_HEADING_STEP_DEG and hold it.
    """
    psi0 = float(HEADING_CMD_DEG)
    t = float(t)

    t_sine_start = float(CMD_HOLD_INITIAL_S)
    t_sine_end = t_sine_start + float(CMD_SINE_DURATION_S)

    if t < t_sine_end:
        return psi0

    return wrap_deg_series(np.array([psi0 + float(CMD_FINAL_HEADING_STEP_DEG)], dtype=float))[0]


def command_profile_pitch_deg(t: float, cfg=None) -> float:
    """
    Time-varying pitch command for this 30 s case.

    The controller tracks pitch angle, not vertical position. To represent the requested
    1.0 m amplitude, 0.25 Hz pitch-direction sine segment, this converts the tangent
    of a vertical sine path into an equivalent pitch command:

        z_up_cmd = A sin(2*pi*f*tau)
    ...
    if t < t_sine_end:
        ...
        vertical_velocity_cmd = amp_m * omega * np.cos(omega * tau)
...
def command_profile_position_m(t: float) -> tuple[float, float, float]:
    """
    Diagnostic command path used for plotting/logging.
    The controller does not directly track this position; it tracks the pitch/yaw
    commands generated by the command profile.
    ...
    if t < t_sine_start:
        ...
        tau = t - t_sine_start
        z_up_cmd = amp_m * np.sin(2.0 * np.pi * float(CMD_SINE_FREQ_HZ) * tau)
    else:
        z_up_cmd = 0.0
...
def plot_mpc_results(df: pd.DataFrame):
    ...
    if "z_up_cmd_m" in df.columns:
        ...
        plt.title("Pitch-Direction Sine Command Diagnostic")
        plt.legend()
