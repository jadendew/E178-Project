...
OUTPUT_CSV = r"sim_out.csv"

# Controller module path (must expose controller(t, state, cfg))
CONTROLLER_MODULE = "controllers.heading_pitch_hold_mlp_alloc"
...
# ============================================================
# MLP control allocation settings
# ============================================================

MLP_ALLOC_ELEVON_MAX_DEG = 25.0

MLP_ALLOC_W_CL = 5.0
MLP_ALLOC_W_CM = 10.0
MLP_ALLOC_W_CN = 1.0

MLP_ALLOC_W_EFFORT = 0.0005
MLP_ALLOC_W_SMOOTH = 0.02
MLP_ALLOC_W_ELEVON_LIMIT = 100.0

MLP_ALLOC_MAXITER = 20
...
def main():
    ...
    cfg.mlp_alloc_elevon_max_deg = float(MLP_ALLOC_ELEVON_MAX_DEG)

    cfg.mlp_alloc_w_cl = float(MLP_ALLOC_W_CL)
    cfg.mlp_alloc_w_cm = float(MLP_ALLOC_W_CM)
    cfg.mlp_alloc_w_cn = float(MLP_ALLOC_W_CN)

    cfg.mlp_alloc_w_effort = float(MLP_ALLOC_W_EFFORT)
    cfg.mlp_alloc_w_smooth = float(MLP_ALLOC_W_SMOOTH)
    cfg.mlp_alloc_w_elevon_limit = float(MLP_ALLOC_W_ELEVON_LIMIT)

    cfg.mlp_alloc_maxiter = int(MLP_ALLOC_MAXITER)

    # Make these available to controller-side alpha/beta clamp
