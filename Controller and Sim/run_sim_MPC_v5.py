...
OUTPUT_CSV = r"sim_out_MLP_MPC_v5.csv"
...
#   v5 is the same v3 setup, but with the sine section removed:
#   0-10 s: hold the script's original commanded heading/pitch
#   10-20 s: command a new steady yaw/heading 90 deg from HEADING_CMD_DEG
#            and hold the original steady pitch command
USE_COMMAND_PROFILE = True
CMD_HOLD_INITIAL_S = 10.0
CMD_FINAL_HEADING_STEP_DEG = 90.0

# Kept only for compatibility with the existing diagnostic helper.
CMD_SINE_DURATION_S = 0.0
CMD_SINE_FREQ_HZ = 0.0
CMD_SINE_AMPLITUDE_M = 0.0
...
SAVE_PLOTS = True
PLOT_OUTPUT_DIR = "mpc_output_plots_v5"
PLOT_DPI = 200

SAVE_ANIMATION = True
ANIMATION_OUTPUT_DIR = "mpc_output_animation_v5"
ANIMATION_FILENAME = "mpc_v5_animation.mp4"  # falls back to .gif if mp4 fails
ANIMATION_FPS = 30
ANIMATION_DPI = 140
...
def command_profile_heading_deg(t: float, cfg=None) -> float:
    """
    Time-varying yaw/heading command for this 20 s v5 case.

    0-10 s: hold the script's original heading command, HEADING_CMD_DEG.
    10-20 s: step yaw/heading by CMD_FINAL_HEADING_STEP_DEG and hold it.
    """
    psi0 = float(HEADING_CMD_DEG)
    t = float(t)

    if t < float(CMD_HOLD_INITIAL_S):
        return psi0
...
def command_profile_pitch_deg(t: float, cfg=None) -> float:
    """
    Pitch command for v5.

    The sine section is removed, so pitch is held at the original v3 steady-state
    command for the full run.
    """
    return float(PITCH_CMD_DEG)


def command_profile_position_m(t: float) -> tuple[float, float, float]:
    """
    Diagnostic command path placeholder for v5.

    There is no sine trajectory command in v5, so z_up_cmd is always zero.
    """
    t = float(t)
