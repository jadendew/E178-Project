...
def animate_pitch_path_stl(df: pd.DataFrame, stride=10, interval_ms=30, trail_len=200,
    ...
    This version uses a fixed world-frame side view:
      - the camera is orthogonal to the WORLD y-axis (not body-following)
      - the frame limits are fixed from the full run so the whole flight path
        always remains visible in the picture
    ...
    # Fixed limits from the entire run so both the commanded and actual paths stay visible.
    x_all = np.r_[xp, x_cmd_world]
    y_all = np.r_[yp, y_cmd_world]
    z_all = np.r_[zp, z_cmd_plot]
    ...
    z_span = max(z_max - z_min, 6.0)

    # Wide static frame in x, tight in y, moderate in z.
    x_pad = 0.06 * x_span
    y_pad = max(0.35, 0.20 * y_span)
    z_pad = 0.18 * z_span

    ax.set_xlim(x_min - x_pad, x_max + x_pad)
    ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ax.set_zlim(z_min - z_pad, z_max + z_pad)
    ax.set_box_aspect((5.2, 0.45, 1.35))

    # Fixed world-frame view: look along the WORLD y-axis.
    ax.view_init(elev=0.0, azim=-90.0)

    axis_len = max(0.15, 0.010 * max(x_span, z_span))
