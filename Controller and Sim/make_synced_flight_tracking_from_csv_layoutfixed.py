...
Notes:
    - This version is tuned to preserve the natural subplot proportions.
    - It avoids aggressive stretching by using a fixed figure aspect ratio.
    - It also increases spacing between the flight panel and the tracking/error panels
      so titles, axis labels, and tick labels do not overlap.
...
# Manual subplot spacing to avoid title / axis label overlap.
SUBPLOT_WSPACE = 0.12
SUBPLOT_HSPACE = 0.24
FIG_TOP = 0.90
FIG_BOTTOM = 0.10
...
def make_animation(df):
    ...
    if SHOW_ERROR_PANEL:
        ...
        fig = plt.figure(figsize=FIGSIZE, constrained_layout=False)
        gs = fig.add_gridspec(1, 2, width_ratios=LEFT_RIGHT_WIDTH_RATIO, wspace=SUBPLOT_WSPACE)
        ax_3d = fig.add_subplot(gs[0, 0], projection="3d")
        ax_track = fig.add_subplot(gs[0, 1])
        ax_err = None

    fig.suptitle("Synchronized MPC Flight Animation and Attitude Tracking", fontsize=13, y=0.975)

    # 3D flight axis
    ax_3d.set_title("MPC v2 Flight Animation", pad=12, fontsize=12)
    ...
    fig.subplots_adjust(left=FIG_LEFT, right=FIG_RIGHT, bottom=FIG_BOTTOM, top=FIG_TOP, wspace=SUBPLOT_WSPACE, hspace=SUBPLOT_HSPACE)
